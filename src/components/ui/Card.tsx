...
const TestimonialCard = React.forwardRef<HTMLDivElement, TestimonialCardProps>(
  ({ content, author, role, company, companyHref, rating, className }, ref) => {
    const companyLabel = companyHref ? (
      <a
        href={companyHref}
        target="_blank"
        rel="noopener noreferrer"
        className="text-navy-600 hover:text-primary-700 underline underline-offset-2 transition-colors"
      >
        {company}
      </a>
    ) : (
    ...
    return (
      <Card ref={ref} className={cn("h-full", className)}>
        ...
        <CardFooter className="flex flex-col items-start space-y-1">
          ...
          {!role && company && (
            <p className="text-sm text-navy-600 turkish-text">{companyLabel}</p>
          )}
        </CardFooter>
      </Card>
    );
